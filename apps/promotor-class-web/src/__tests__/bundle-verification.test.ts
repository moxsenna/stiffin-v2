...
describe('P0-5 — Production Frontend Build Configuration & Bundle Invariants', () => {
  ...
  it('3. Production source files contain zero MockStateStore imports', () => {
    function scanDir(dirPath: string): string[] {
      let results: string[] = [];
      if (!fs.existsSync(dirPath)) return results;
      for (const file of fs.readdirSync(dirPath)) {
    ...
    for (const file of allFiles) {
      ...
      assert.strictEqual(
        /import\s+.*MockStateStore/.test(content),
        false,
        `Production source file ${rel} MUST NOT import MockStateStore`
      );
      assert.strictEqual(
        content.includes('/adapters/mock/'),
        false,
        `Production source file ${rel} MUST NOT import from /adapters/mock/`
      );
    }
  });
});
