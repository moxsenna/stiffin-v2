...
const CANONICAL = {
  ...
  '0010_talira_billing_and_commerce': 'c7bd9db8c6d5dd205e873e84d33e33862703d493c249cf145906b81a676a5748',
} as const;
...
const CRLF_KNOWN = {
  ...
  '0010_talira_billing_and_commerce': '02aa94e91d977af165c16c49fa07a71ac3b39acc94f3e386573d6f65b142a2cf',
} as const;
...
function migrationBytes(): { name: string; lf: string }[] {
  ...
  return [
    ...
    '0010_talira_billing_and_commerce',
  ].map((tag) => {
...
describe('B2/B3/B6/B6.1/B4/B5 — cross-platform migration fingerprint (canonical LF)', () => {
  ...
  it('canonical LF 0010 -> c7bd9db8... (Git/LF fingerprint)', () => {
    const [, , , , , , , , , , m10] = migrationBytes();
    assert.strictEqual(m10.name, '0010_talira_billing_and_commerce');
    assert.strictEqual(sha256(m10.lf), CANONICAL['0010_talira_billing_and_commerce']);
  });

  it('same content as CRLF -> known non-canonical Windows hashes (diagnostic only)', () => {
    const [m0, m1, m2, m3, m4, m5, m6, m7, m8, m9, m10] = migrationBytes();
    ...
    assert.strictEqual(sha256(crlf10), CRLF_KNOWN['0010_talira_billing_and_commerce'], 'CRLF 0010 must hash to the known Windows diagnostic hash');
    ...
    assert.notStrictEqual(CRLF_KNOWN['0010_talira_billing_and_commerce'], CANONICAL['0010_talira_billing_and_commerce']);
  });
