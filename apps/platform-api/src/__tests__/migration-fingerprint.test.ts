...
function migrationBytes(): { name: string; lf: string }[] {
  ...
  return ['0000_modern_hydra', '0001_material_king_bedlam', '0002_heavy_scarlet_witch', '0003_smart_titania'].map((tag) => {
...
describe('B2/B3/B6 — cross-platform migration fingerprint (canonical LF)', () => {
  ...
  it('same content as CRLF -> known non-canonical Windows hashes (diagnostic only)', () => {
    const [m0, m1, m2, m3] = migrationBytes();
