  ...
  return {
    ...
    async assertCanPublishProgram(organizationId: string): Promise<void> {
      ...
      if (access.usage.publishedPrograms >= access.limits.maxPublishedPrograms) {
        throw new DomainError(
          'PLAN_LIMIT_REACHED',
          `Batas program terpublikasi (${access.limits.maxPublishedPrograms}) untuk paket ${access.plan.name} telah tercapai. Tingkatkan paket ke Talira Solo untuk mempublikasikan hingga 10 program.`,
          {
    ...
    async assertCanUsePaidPrograms(organizationId: string): Promise<void> {
      ...
      if (!access.limits.canUsePaidPrograms) {
        throw new DomainError(
          ...
          'Penjualan program kelas berbayar hanya tersedia di paket Talira Solo. Silakan upgrade paket Anda untuk mengaktifkan fitur ini.',
          {
    ...
    async assertCanCustomizeStorefront(organizationId: string): Promise<void> {
      ...
      if (!access.limits.canCustomizeStorefront) {
        throw new DomainError(
          'FEATURE_REQUIRES_UPGRADE',
          'Kustomisasi tema dan branding storefront lanjutan hanya tersedia di paket Talira Solo.',
          {
    ...
    async assertCanAddContact(organizationId: string): Promise<void> {
      ...
      if (access.usage.contacts >= access.limits.maxContacts) {
        throw new DomainError(
          'PLAN_LIMIT_REACHED',
          `Batas kontak CRM (${access.limits.maxContacts}) untuk paket ${access.plan.name} telah tercapai. Tingkatkan paket ke Talira Solo untuk mengelola hingga 2.500 kontak.`,
          {
            feature: 'contacts',
            currentPlan: access.plan.code,
            requiredPlan: 'SOLO',
