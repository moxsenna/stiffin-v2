...
### Video Provider Decision

V0.1 hanya mendukung:

```text
YouTube Unlisted
```

Tidak ada upload/hosting/transcoding video di PromotorClass V0.1.

Promotor menempelkan URL YouTube Unlisted ke lesson.
...
Untuk lesson video V0.1:

```text
video_provider = youtube
```

Server harus memvalidasi URL YouTube dan mengekstrak `video_external_id`.

Completion rules schema boleh future-ready:
...
Namun **V0.1 hanya mengaktifkan**:

```text
manual
```

Video selesai diputar **tidak otomatis** menyelesaikan lesson.
...
Contact identity is owned by Shared Platform Core.

When a learner registers:

1. normalize phone using the shared E.164 normalizer,
2. search canonical Contact in the same organization,
3. optionally check normalized email,
4. if matched:
   - reuse existing `contact_id`,
5. if not matched:
   - create canonical Contact through Shared Core.
...
Phone is the primary V0.1 identity key.
...
PromotorClass does not create a separate learner person record.
...
PromotorClass integrates through the shared Promotor Platform foundation and versioned adapters.
...
user
contact
E.164 phone identity
organization membership/auth context
```

PromotorClass references these canonical records.

It must not create independent copies merely because a Contact becomes a learner.

## PromotorFlow Owns

```text
contact business lifecycle
...
Canonical `NextAction` exists only in PromotorFlow.
...
PromotorClass may calculate:

```text
recommended_next_step
```

When that recommendation must become operational work:
...
↓
canonical PromotorFlow NextAction
```

PromotorClass must not keep a second canonical `next_actions` table.
...
```text
read Flow contact business context
read lifecycle stage
read generalized assessment status
request Flow Next Action
project meaningful learning activity into Flow
open contact in PromotorFlow
```
...
own canonical Next Action
replace PromotorFlow lead management
copy full Flow activity model
```
...
PDF/document upload
external link
YouTube Unlisted video URL
```

## Video Policy V0.1

Satu-satunya provider video:
...
```text
validate YouTube host
extract video ID
store original URL
store provider = youtube
store external ID
...
## YouTube Branding Constraint

PromotorClass **tidak menjanjikan white-label video playback** selama provider masih YouTube.

Branding, controls, atau elemen resmi YouTube yang ditampilkan oleh official embed tidak boleh ditutupi, di-crop, atau disamarkan melalui overlay/CSS.

Design harus membuat surrounding lesson UI tetap bersih, tetapi tidak memodifikasi official player di luar parameter yang didukung YouTube.
...
Learner menekan `Complete lesson`.

YouTube playback event tidak menjadi dependency untuk progress V0.1.

## Future Provider Abstraction

Schema harus future-ready:
...
sehingga provider white-label dapat ditambahkan di masa depan tanpa migrasi besar terhadap lesson model.
...
# 59A. Locked Product Decision — Video Strategy
...
V0.1 video strategy is locked as:
...
Video hosting:
Not owned by PromotorClass
```

Rationale:

- dramatically reduces infrastructure complexity,
- no video upload pipeline,
- no transcoding,
- no CDN/video storage cost,
- easier for promotor to operate,
- sufficient for early product validation.

Trade-off accepted:

> learner may still see YouTube branding/UI elements controlled by YouTube.

This is considered acceptable for V0.1.

A future white-label provider may be added behind the provider abstraction if validated user demand justifies the additional cost and infrastructure.
