...
# 69. Future LMS Architecture Seam

LMS should become another module in the same platform:
...
not a separate LMS user/customer entity.

---

# 70. Future LMS Event Flow
...
must be stable core primitives.

---

# 71. Future Shared Platform Model
...
```text
               ...
               ┌─────┴─────┐
               ▼           ▼
        PromotorFlow      Learning
        CRM lifecycle     LMS lifecycle
               │           │
               └─────┬─────┘
                     ▼
                  Activity
...
> PromotorFlow uses a modular monolith. Keep business logic out of UI components. All private records are organization-scoped. Contact is the canonical person identity across prospect, client, and future LMS lifecycle. Next Actions are persistent domain records. Activity is a durable business timeline. Booking completion must atomically update booking, contact lifecycle, activities, and aftercare. Do not introduce microservices, biometric data, WhatsApp auto-send, or separate lead/client tables.
