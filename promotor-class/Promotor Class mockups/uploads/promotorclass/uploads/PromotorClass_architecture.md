...
│                      DOMAIN MODULES                          │
│                                                              │
│ Organizations │ Contacts │ Programs │ Lessons               │
...
```text
Learner != separate person record
```
...
Next Action is business-oriented output.
...
# 30. Next Action Deduplication

Do not create repeated pending action for same trigger.

Suggested uniqueness logic:
...
PromotorClass must not directly manipulate PromotorFlow internals.

Use adapter:

```ts
interface PromotorFlowAdapter {
  getContactContext(contactId: string): Promise<ContactContext>;
  getAssessmentStatus(contactId: string): Promise<AssessmentStatus>;
  createNextAction(input: NextActionInput): Promise<void>;
...
# 52. PromotorFlow V0.1 Modes

Two possible deployments.

## Mode A — Shared Database

PromotorFlow and PromotorClass share:
...
Recommended if both are developed together initially.

---

## Mode B — Separate Service

PromotorClass communicates via adapter/API.

Use when products are deployed independently.
...
if shared platform is used.

If separate systems later:
...
PromotorClass must remain usable if PromotorFlow is unavailable.

Learning actions should still work.

If external next action sync fails:

```text
save local pending sync
retry later
```
...
because CRM integration failed.
...
Initial adapter can map directly to local/shared models.
...
without changing caller modules.

---

# 97. Integration Failure Queue

If using HTTP adapter later:
...
Not necessary if shared DB mode.
...
```text
Contact is canonical customer identity across learning and business workflows.
```
